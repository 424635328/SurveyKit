...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchResults() {
        ...
        try {
            ...
            const [structureRes, resultsRes] = await Promise.all([
                fetch(`/api/survey-details.mjs?id=${surveyId}`, { headers: { 'Authorization': `Bearer ${token}` } }),
    ...
    function renderResults(data) {
        ...
        const orderedQuestions = data.structure.questions.flatMap(part => part.questions);
        ...
        data.submissions.forEach(submission => {
            ...
            // 添加提交时间
            const submittedAt = new Date(submission.submittedAt).toLocaleString('zh-CN', {
    ...
    function downloadCSV() {
        ...
        const orderedQuestions = fullResultsData.structure.questions.flatMap(part => part.questions);
