document.addEventListener('DOMContentLoaded', () => {
    ...
    if (pageSurveyTitleElement) {
        pageSurveyTitleElement.textContent = surveyTitle ? `“${surveyTitle}”` : '您的问卷';
    }
    ...
    if (qrcodeDiv && surveyId) {
        ...
        if (qrcodeDiv) qrcodeDiv.innerHTML = '<p class="text-red-500 text-center text-sm mt-4">无法生成二维码：缺少问卷ID。</p>';
    }

    // 复制链接功能
    if (copyButton) { // 确保按钮存在才添加事件监听
